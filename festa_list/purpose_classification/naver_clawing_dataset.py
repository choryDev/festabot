...
class Naver_clawing_for_keyword:
            ...
            for item_index in range(0, len(response_body_dict['items'])):
                try:
                    ...
                    for mainFrame in post_soup.select('iframe#mainFrame'):
                        ...
                        Naver_clawing_for_keyword.get_html_tag(self, blog_post_soup, "p")  # 내용을 계속 이어 붙인다
                except:
                    item_index += 1
    ...
    def main(self):
        ...
        blog_count = Naver_clawing_for_keyword.get_blog_count(self, query, display)
        for start_index in range(start, blog_count + 1, display):
            Naver_clawing_for_keyword.get_blog_post(self, query, display, start_index, sort)
...
#doc2vec_dataset_path = '../../option/relataion_festa/Doc2Vec/dataset/naver_doc2vec_dataset'
doc2vec_dataset_path = '/home/ubuntu/festabot/option/relation_festa/Doc2Vec/dataset/'
file = open(doc2vec_dataset_path+time_title+'.txt','w')

for id, title in db_obj:
    blog_summary = ""
    sents = Naver_clawing_for_keyword(title).main()  # id, title
    keysents = summarizer.summarize(sents, topk=500)
    id_list.append(id)

    for v in keysents:  #한 블로그에서 요약이 된 내용을 이어 붙이기
        blog_summary += v[2]
        file.write(v[2] + '␞' + str(id) + '\n')
    summary_list.append(blog_summary)   #요약된 내용을 담기

file.close()
obj = Keyword_extractor(id_list, summary_list).make_obj()

with open('./word_freq_dir/word_freq' + datetime.today().strftime("%Y%m%d") + '.json', 'w', encoding='utf-8') as make_file:
